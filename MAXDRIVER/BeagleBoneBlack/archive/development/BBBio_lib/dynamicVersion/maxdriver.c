...
void initMax(S_MAX * max, uint8_t connector, uint8_t clkpin, uint8_t cspin, uint8_t datapin, uint8_t nrmax)
{
	...
	iolib_init();
	iolib_setdir(max->connector,max->clkpin,BBBIO_DIR_OUT);
	iolib_setdir(max->connector,max->cspin,BBBIO_DIR_OUT);
	iolib_setdir(max->connector,max->datapin,BBBIO_DIR_OUT);
